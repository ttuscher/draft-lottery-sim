...
export default function FastDrawBoard({ triggerRef }: FastDrawBoardProps) {
  ...
  const [odds, setOdds] = useState<{
    pickSlotOdds: Record<number, Record<string, number>>;
    draw1Odds: Record<string, number>;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    setOdds({ pickSlotOdds, draw1Odds });
  }, [dynamicData]);
  ...
  const getTeamOdds = (abbrev: string) => {
    if (!odds) return { firstOvr: 0, secondOvr: 0, d1: 0, d2: 0 };
          ...
          <table className={`text-left border-collapse ${expanded ? 'min-w-[800px] w-full' : 'w-full'}`}>
            ...
            <tbody>
              ...
              {displayLotteryTeams.map((slot, index) => {
                ...
                const hasFallen = result && slot.seed < pickNum;
                ...
                const combos = lotteryTeams.find(t => t.team.abbreviation === abbrev)?.combinations ?? 0;
                    ...
                    <td className={`py-1.5 px-1 sm:px-2 ${stickyTeamClass} max-w-[120px] sm:max-w-[160px] md:max-w-[200px]`} style={{ backgroundColor: 'inherit' }}>
                      <div className="flex items-center gap-1.5 md:gap-2 w-full">
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          {result && !expanded && (
                            <span className="text-[7px] sm:text-[8px] text-gray-500 uppercase tracking-widest">
                              SEED #{slot.seed}
                            </span>
                          )}
                        </div>
