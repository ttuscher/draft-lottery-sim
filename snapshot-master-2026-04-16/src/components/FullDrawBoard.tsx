              ...
              <tbody>
                ...
                {history.map((row, idx) => {
                  ...
                  if (isRedraw) {
                    ...
                    line1 = `${resultTeam.team.abbreviation} WINS DRAW ${drawNum}`;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-8">
          ...
          <div className="w-full bg-white border-4 border-black p-3 md:p-4 shadow-[8px_8px_0px_rgba(0,0,0,1)]">
            ...
            <div>
              <table className="w-full text-left border-collapse table-fixed">
                ...
                <thead>
                  <tr className="bg-[#B8F6FA] border-b-4 border-black text-[9px] sm:text-[11px] md:text-sm">
                    <th className="py-1 px-1 text-center">RANK</th>
                    <th className="py-1 px-1 sm:px-2 text-left">TEAM</th>
                ...
                <tbody>
                  {(() => {
                    ...
                    return liveOdds.map((odds) => {
                      ...
                      const delta = displayWin - baseline;
                      ...
                      return (
                        ...
                        >
                          <td className={`py-[7px] px-1 text-center font-bold ${isD1Winner ? 'text-gray-400' : 'text-black'}`}>
                            {isD1Winner ? 'D1' : rank}
                          </td>
                          <td className="py-[7px] px-1 sm:px-2">
                          ...
                          <td className={`py-[7px] px-1 text-center font-bold ${isD1Winner ? 'text-gray-400' : ''}`}>
                            {odds.remainingCombos}
                          </td>
                          <td className={`py-[7px] px-1 text-right font-bold ${isD1Winner ? 'text-gray-400' : ''}`}>
                            {displayWin > 0 ? (
                          ...
                          <td className={`py-[7px] px-1 text-right pr-2 font-bold ${isD1Winner ? 'text-gray-400' : ''}`}>
                            {Math.abs(delta) < 0.05 ? (
          ...
          <div className="w-full bg-white border-4 border-black p-3 md:p-4 shadow-[8px_8px_0px_rgba(0,0,0,1)] flex flex-col">
            ...
            <div className="relative mb-2">
              ...
              >
                <span className="text-[9px] sm:text-[11px] md:text-sm lg:text-xs leading-snug text-left truncate">
              ...
              {teamDropdownOpen && (
                <div className="absolute top-full left-0 w-full mt-2 bg-black border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] z-50 max-h-[200px] overflow-y-auto custom-scrollbar flex flex-col">
                  {allTeamCodes
                    .filter(code => code !== draw1WinnerCode)
                    .map(code => {
            ...
            {(() => {
              ...
              const ballsAlive = impact.filter(i => i.status === 'REMAINING_ALIVE' || i.status === 'DRAWN_MATCH').length;
              return (
                <div className="flex mb-1 flex-grow">
                  <div className="flex-1 flex flex-col items-center justify-center py-1 bg-[#E5FCFD]">
                    ...
                    <span className="text-[6px] sm:text-[7px] uppercase font-bold text-gray-500 leading-tight">BALLS ALIVE</span>
                  </div>
                  <div className="flex-1 flex flex-col items-center justify-center py-1 bg-[#E5FCFD]">
                    <span className="text-sm md:text-lg font-bold text-black leading-tight">{currentWin.toFixed(1)}%</span>
                    <span className="text-[6px] sm:text-[7px] uppercase font-bold text-gray-500 leading-tight">CURRENT WIN %</span>
                  </div>
                  <div className="flex-1 flex flex-col items-center justify-center py-1 bg-[#E5FCFD]">
                    <span className={`text-sm md:text-lg font-bold leading-tight ${deltaColor}`}>{deltaStr}</span>
                    <span className="text-[6px] sm:text-[7px] uppercase font-bold text-gray-500 leading-tight">VS ORIGINAL</span>
                  </div>
                </div>
              );
            })()}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {currentOrder.map((slot, index) => {
                  ...
                  const isDraw1Winner = draw1Winner?.team.abbreviation === slot.team.abbreviation;
                  const isDraw2Winner = draw2Winner?.team.abbreviation === slot.team.abbreviation;
                  const isWinner = isDraw1Winner || isDraw2Winner;

                  const isSeed1 = slot.seed === 1;
                  const isSeed1Consolation = isSeed1 && isDraw2Winner && !isDraw1Winner;

                  const hasFallen = slot.seed < pickNum;

                  const change = slot.seed - pickNum;

                  let numClass = 'text-black [text-shadow:2px_2px_0_#fff]';
                  if (change > 0) {
                    numClass = 'text-black [text-shadow:2px_2px_0_#FFCC00]';
                  } else if (change < 0) {
                  ...
                  if (change > 0) {
                    changeLabel = `▲ ${change}`;
                    changeColor = 'text-[#FFCC00] [text-shadow:1.5px_1.5px_0_#000]';
                  } else if (change < 0) {
                  ...
                  return (
                    <tr key={slot.team.abbreviation} className="border-b-2 border-gray-200 hover:bg-[#E5FCFD] transition-colors">
                      ...
                      <td className="py-1.5 px-1 sm:px-2">
                        <div className="flex items-center gap-2 md:gap-3 w-full">
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-[8px] sm:text-[9px] text-gray-500 uppercase tracking-widest">
                              SEED #{slot.seed}
                            </span>
                          </div>
                        </div>
                      ...
                      <td className="py-1.5 px-1">
                        <div className="flex justify-center items-center h-full">
                          <span className={`font-bold text-xs sm:text-sm md:text-base ${changeColor}`}>
                            {changeLabel}
                          </span>
                        </div>
                      </td>
