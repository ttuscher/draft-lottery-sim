...
// (Accounts for the 10-spot rule cascading)
...
export default function LandingPage() {
  // Extract the live standings and base odds directly from the official 2025 configuration
            ...
            <tbody>
              {standings.map((row) => (
                <tr key={row.team.abbreviation} className="border-b-2 border-gray-200 text-[10px] md:text-xs hover:bg-[#96EDF6] transition-colors">
                  ...
                  {/* SEED */}
                  <td className="p-3 text-center font-bold text-[#E2231A]">
                    {row.seed}
                  </td>
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                      {row.team.logoLight && (
                        <img 
                          ...
                          // Increased size to w-8 h-8 (mobile) and md:w-10 md:h-10 (desktop)
                          // CRT Arcade Filters: Hyper-saturate, high contrast, slight sepia age, chunky 3px shadow
                  ...
                  <td className="p-3 text-right font-bold">
                    {row.odds1 > 0 ? `${row.odds1.toFixed(1)}%` : '-'}
                  </td>
                  
                  {/* 2ND OVERALL ODDS */}
                  <td className="p-3 text-right font-bold">
                    {row.odds2 > 0 ? `${row.odds2.toFixed(1)}%` : '-'}
                  </td>
