...
export default function FullDrawBoard({ setActionText, triggerRef }: FullDrawBoardProps) {
 ...
 {row.balls.map((b, i) => (
 <div key={i} className="shrink-0 w-6 h-6 md:w-10 md:h-10 flex items-center justify-center rounded-full border-2 md:border-4 border-[#FFCC00] bg-[#FFCC00] text-black font-bold text-micro shadow-[0_0_10px_rgba(255,204,0,0.8)]">
 {b}
 </div>
 ))}
 </div>
 ...
 if (ball) {
 return (
 <div key={`slot-${slotIndex}`} className="shrink-0 w-6 h-6 md:w-10 md:h-10 flex items-center justify-center rounded-full border-2 md:border-4 border-white bg-white text-black font-bold text-micro shadow-[0_0_10px_rgba(255,255,255,0.8)] animate-in zoom-in duration-200">
 {ball}
 </div>
 );
 } else {
 return (
 <div key={`slot-${slotIndex}`} className="shrink-0 w-6 h-6 md:w-10 md:h-10 flex items-center justify-center rounded-full border-2 md:border-4 border-solid border-gray-400 bg-transparent text-gray-400 font-bold text-micro shadow-[0_0_8px_rgba(156,163,175,0.8)]">
 ?
 </div>
 );
 }
 ...
 <span className="text-gray-400 font-bold text-micro uppercase tracking-widest animate-pulse">READY TO BEGIN...</span>
 ) : (
 <span className="text-gray-400 font-bold text-micro uppercase tracking-widest animate-pulse">DRAWING...</span>
 )}
 </div>
 </td>
 </tr>
